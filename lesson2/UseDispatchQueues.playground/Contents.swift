...
let mySerialQueue = DispatchQueue(label: "com.juno.serial") //default is serial so only need to put a label
...
// In a serial queue task2 has to wait for task1 to finish
sleep(2)
...
let workerQueue = DispatchQueue(label: "com.juno.worker", attributes: .concurrent)
